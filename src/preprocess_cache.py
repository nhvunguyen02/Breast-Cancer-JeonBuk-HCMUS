...
def _process_one(task):
    """task = (src_path, dst_path, view_code, side, remove_pectoral)."""
    src, dst, view_code, side, remove_pectoral = task
    if os.path.exists(dst):
        return dst
    gray = np.asarray(Image.open(src).convert("L"), dtype=np.float32)
    out = preprocess_view(gray, view=view_code, side=side, remove_pectoral=remove_pectoral)
...
def main():
    ...
    for i, row in df.iterrows():
        ...
        for view in VIEW_NAMES:
            ...
            tasks.append((src, dst, view_code, side, args.remove_pectoral))
